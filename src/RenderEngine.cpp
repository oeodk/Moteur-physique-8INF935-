...
constexpr float HEIGH_FACTOR = 2;
...
RenderEngine::RenderEngine()
{
	...
	for (int i = 0; i < _TERRAIN_DIVISION; i++)
	{
		for (int j = 0; j < _TERRAIN_DIVISION; j++)
		{
			...
			_terrain.addVertex(Vector3D(-(_TERRAIN_SIZE / 2) + _TERRAIN_SIZE * (i / static_cast<float>(_TERRAIN_DIVISION)), color * HEIGH_FACTOR, -(_TERRAIN_SIZE / 2) + _TERRAIN_SIZE * (j / static_cast<float>(_TERRAIN_DIVISION))));
			//_terrain.addNormal(Vector3D(0,1,0));
			_terrain.addColor(ofColor(63, 155, 11));
		}
	}
