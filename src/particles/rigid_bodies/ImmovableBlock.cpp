...
ImmovableBlock::ImmovableBlock(const Vector3D& init_pos, const Vector3D& init_vel, const Quaternion& init_orientation, const Vector3D& init_angular_vel, const Vector3D& base_force_for_rotation, const Vector3D& application_point)
	: RigidBody(init_pos, constants::EMPTY_VECTOR3D, INT_MAX, 100, init_orientation, init_angular_vel, "placing_block.mp3")
{
	if (!mesh_init_)
	{
		mesh_init_ = true;
		...
		for (auto& c : _cube_mesh.getColors())
		{
			c.r = 1 - c.r;
			c.g = 1 - c.g;
			c.b = 1 - c.b;
...
void ImmovableBlock::getInertiaMoment(Matrix3& mat) const
{
